...
`OPENAI_NOVEL_MODEL` 用於主線小說生成；`OPENAI_NOVEL_QUICK_MODEL` 只在新奇度不足需要低成本重試時使用；`OPENAI_NOVEL_FALLBACK_MODEL` 會在主模型尚未開通、模型名稱錯誤或參數不支援時自動接手。API 模型 ID 請使用 `gpt-5.5` 這類 slug，不要填 ChatGPT 介面名稱如 `gpt-5.5-thinking`。程式會把常見舊填法自動正規化，但 Vercel 建議直接填正確 slug。
